        ...
        public static void Initialize(IServiceProvider serviceProvider)
        {
                ...
                serviceProvider.GetRequiredService<DbContextOptions<CanadaGamesContext>>()))
            {
                ...
                if (!context.Placements.Any())
                {
                    ...
                    {
                        //Note: added the .OrderBy(r => random.Next()).ToArray(); to try and avoid
                        //Athletes getting the same placemnt each time.
                        int i = 1;
                        var competitorIDs = (from a in context.Athletes
                                            where a.GenderID == e.GenderID && a.SportID == e.SportID
                                            select new { a.ID }).ToArray();
                        var compIDs = competitorIDs.OrderBy(r => random.Next()).ToList();
                        foreach (var c in compIDs)
                        {
